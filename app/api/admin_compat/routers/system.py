...
async def page_roles(params: RoleQuery, request: Request):
    return JsonResponse(
        await system_service.page_roles(params, get_admin_user_from_request(request))
    )


@router.post("/role/list", summary="查询角色列表", tags=["管理台兼容层-系统管理"])
async def list_roles(request: Request, params: RoleQuery = Body(default_factory=RoleQuery)):
    return JsonResponse(
        await system_service.list_roles(params, get_admin_user_from_request(request))
    )


@router.post("/role/add", summary="新增角色", tags=["管理台兼容层-系统管理"])
@log_api
async def add_role(form: RoleForm, request: Request):
    return JsonResponse(
        await system_service.add_role(form, get_admin_user_from_request(request))
    )


@router.post("/role/update", summary="修改角色", tags=["管理台兼容层-系统管理"])
@log_api
async def update_role(form: RoleForm, request: Request):
    return JsonResponse(
        await system_service.update_role(form, get_admin_user_from_request(request))
    )
...
async def remove_role(role_id: int, request: Request):
    return JsonResponse(
        await system_service.remove_role(role_id, get_admin_user_from_request(request))
    )


@router.post("/role/remove/batch", summary="批量删除角色", tags=["管理台兼容层-系统管理"])
@log_api
async def remove_roles(role_ids: list[int], request: Request):
    return JsonResponse(
        await system_service.remove_roles(role_ids, get_admin_user_from_request(request))
    )


@router.post("/role-menu/list/{role_id}", summary="查询角色菜单", tags=["管理台兼容层-系统管理"])
async def list_role_menus(role_id: int, request: Request):
    return JsonResponse(
        await system_service.list_role_menus(role_id, get_admin_user_from_request(request))
    )


@router.post("/role-menu/update/{role_id}", summary="更新角色菜单", tags=["管理台兼容层-系统管理"])
@log_api
async def update_role_menus(role_id: int, menu_ids: list[int], request: Request):
    return JsonResponse(
        await system_service.update_role_menus(
            role_id,
            menu_ids,
...
async def page_users(params: UserQuery, request: Request):
    return JsonResponse(
        await system_service.page_users(params, get_admin_user_from_request(request))
    )


@router.post("/user/list", summary="查询用户列表", tags=["管理台兼容层-系统管理"])
async def list_users(request: Request, params: UserQuery = Body(default_factory=UserQuery)):
    return JsonResponse(
        await system_service.list_users(params, get_admin_user_from_request(request))
    )
...
async def get_user_detail(user_id: int, request: Request):
    return JsonResponse(
        await system_service.get_user_detail(user_id, get_admin_user_from_request(request))
    )


@router.post("/user/add", summary="新增用户", tags=["管理台兼容层-系统管理"])
@log_api
async def add_user(form: UserForm, request: Request):
    return JsonResponse(
        await system_service.add_user(form, get_admin_user_from_request(request))
    )


@router.post("/user/update", summary="修改用户", tags=["管理台兼容层-系统管理"])
@log_api
async def update_user(form: UserForm, request: Request):
    return JsonResponse(
        await system_service.update_user(form, get_admin_user_from_request(request))
    )
...
async def check_user_existence(form: ExistenceCheckForm, request: Request):
    return JsonResponse(
        await system_service.check_user_existence(
            form,
            get_admin_user_from_request(request),
        )
    )
...
async def page_organizations(params: OrganizationQuery, request: Request):
    return JsonResponse(
        await system_service.page_organizations(params, get_admin_user_from_request(request))
    )
...
async def list_organizations(
    ...
    return JsonResponse(
        await system_service.list_organizations(params, get_admin_user_from_request(request))
    )
...
async def list_organizations_tree(
    ...
    return JsonResponse(
        await system_service.list_organizations_tree(params, get_admin_user_from_request(request))
    )


@router.post("/organization/add", summary="新增机构", tags=["管理台兼容层-系统管理"])
@log_api
async def add_organization(form: OrganizationForm, request: Request):
    return JsonResponse(
        await system_service.add_organization(form, get_admin_user_from_request(request))
    )


@router.post("/organization/update", summary="修改机构", tags=["管理台兼容层-系统管理"])
@log_api
async def update_organization(form: OrganizationForm, request: Request):
    return JsonResponse(
        await system_service.update_organization(form, get_admin_user_from_request(request))
    )


@router.post("/organization/remove/{organization_id}", summary="删除机构", tags=["管理台兼容层-系统管理"])
@log_api
async def remove_organization(organization_id: int, request: Request):
    return JsonResponse(
        await system_service.remove_organization(
            organization_id,
            get_admin_user_from_request(request),
        )
    )
...
async def page_login_records(params: LoginRecordQuery, request: Request):
    return JsonResponse(
        await system_service.page_login_records(params, get_admin_user_from_request(request))
    )
...
async def list_login_records(
    request: Request,
    params: LoginRecordQuery = Body(default_factory=LoginRecordQuery),
):
    return JsonResponse(
        await system_service.list_login_records(params, get_admin_user_from_request(request))
    )


@router.post("/operation-record/page", summary="分页查询操作日志", tags=["管理台兼容层-系统管理"])
async def page_operation_records(params: OperationRecordQuery, request: Request):
    return JsonResponse(
        await system_service.page_operation_records(
            params,
            get_admin_user_from_request(request),
        )
    )
...
async def list_operation_records(
    ...
    return JsonResponse(
        await system_service.list_operation_records(
            params,
            get_admin_user_from_request(request),
        )
    )
